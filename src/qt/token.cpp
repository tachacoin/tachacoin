...
bool ToHash160(const std::string& strTachacoinAddress, std::string& strHash160)
{
    CTxDestination tachacoinAddress = DecodeDestination(strTachacoinAddress);
    if(!IsValidDestination(tachacoinAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&tachacoinAddress);
    if(keyid){
...
bool ToTachacoinAddress(const std::string& strHash160, std::string& strTachacoinAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination tachacoinAddress = keyid;
    if(IsValidDestination(tachacoinAddress)){
        strTachacoinAddress = EncodeDestination(tachacoinAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToTachacoinAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToTachacoinAddress(tokenEvent.receiver, tokenEvent.receiver);
