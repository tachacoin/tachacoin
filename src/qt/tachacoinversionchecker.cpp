...
#define paternVersion "tachacoin-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

TachacoinVersionChecker::TachacoinVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

TachacoinVersionChecker::~TachacoinVersionChecker()
{

}

bool TachacoinVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> TachacoinVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(TACHACOIN_RELEASES)));
    QEventLoop event;
...
Version TachacoinVersionChecker::getMaxReleaseVersion()
{
