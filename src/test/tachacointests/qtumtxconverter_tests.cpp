...
void checkResult(bool isCreation, std::vector<TachacoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    TachacoinTxConverter converter(transaction, NULL);
    ExtractTachacoinTX tachacoinTx;
    BOOST_CHECK(converter.extractionTachacoinTransactions(tachacoinTx));
    std::vector<TachacoinTransaction> result = tachacoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(tachacointxconverter_tests, TestingSetup)
