...
inline void initState(){
    ...
    const std::string dirTachacoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<TachacoinState>(new TachacoinState(dev::u256(0), TachacoinState::openDB(dirTachacoin, hashDB, dev::WithExisting::Trust), dirTachacoin + "/tachacoinDB", dev::eth::BaseState::Empty));
...
inline dev::Address createTachacoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline TachacoinTransaction createTachacoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    TachacoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = TachacoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = TachacoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<TachacoinTransaction> txs){
