...
const char * const BITCOIN_CONF_FILENAME = "tachacoin.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Tachacoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Tachacoin
    // Mac: ~/Library/Application Support/Tachacoin
    // Unix: ~/.tachacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tachacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tachacoin";
#else
    // Unix
    return pathRet / ".tachacoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Tachacoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Tachacoin Core developers";
    }
