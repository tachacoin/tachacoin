...
void TachacoinDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool TachacoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule TachacoinDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t TachacoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t TachacoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t TachacoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t TachacoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool TachacoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void TachacoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void TachacoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void TachacoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void TachacoinDGP::createParamsInstance(){
...
dev::Address TachacoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void TachacoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TachacoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void TachacoinDGP::parseStorageOneUint64(uint64_t& value){
...
void TachacoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule TachacoinDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
