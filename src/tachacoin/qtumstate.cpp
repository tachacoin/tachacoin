...
TachacoinState::TachacoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = TachacoinState::openDB(_path + "/tachacoinDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

TachacoinState::TachacoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute TachacoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, TachacoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createTachacoinAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            tachacoin::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> TachacoinState::vins() const // temp
{
...
void TachacoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* TachacoinState::vin(dev::Address const& _a) const
{
    return const_cast<TachacoinState*>(this)->vin(_a);
}

Vin* TachacoinState::vin(dev::Address const& _addr)
{
...
// void TachacoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void TachacoinState::kill(dev::Address _addr)
{
...
void TachacoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void TachacoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void TachacoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void TachacoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void TachacoinState::validateTransfersWithChangeLog(){
