...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setTachacoinSchedule(tachacoinDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = tachacoinDGP.getBlockSize(nHeight);
    minGasPrice = tachacoinDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = tachacoinDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    TachacoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractTachacoinTX resultConverter;
    if(!convert.extractionTachacoinTransactions(resultConverter)){
    ...
    for(TachacoinTransaction tachacoinTransaction : tachacoinTransactions){
        txGas += tachacoinTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + tachacoinTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(tachacoinTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, tachacoinTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "tachacoinstake";
