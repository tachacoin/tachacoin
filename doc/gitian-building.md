...
*Setup instructions for a Gitian build of Tachacoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Tachacoin
...
- [Building Tachacoin Core](#building-tachacoin-core)
...
Clone the git repositories for tachacoin and Gitian.
...
```bash
sudo cp /home/debian/tachacoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the tachacoin repository under 'Fetch and create inputs' to install sources which require
...
To build Tachacoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the tachacoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/tachacoin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/tachacoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit tachacoin=${COMMIT} --url tachacoin=${URL} ../tachacoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit tachacoin=${COMMIT} --url tachacoin=${URL} ../tachacoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit tachacoin=${COMMIT} --url tachacoin=${URL} ../tachacoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-tachacoin:
./bin/gbuild --commit tachacoin=${COMMIT},cpp-eth-tachacoin=${ETHCOMMIT} --url tachacoin=${URL},cpp-eth-tachacoin=${ETHURL} ../tachacoin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the tachacoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../tachacoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/tachacoin/tachacoin-detached-sigs.git

BTCPATH=/some/root/path/tachacoin
SIGPATH=/some/root/path/tachacoin-detached-sigs

./bin/gbuild --url tachacoin=${BTCPATH},signature=${SIGPATH} ../tachacoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/tachacoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/tachacoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/tachacoin-osx-build.assert
```
...
[tachacoin/gitian.sigs](https://github.com/tachacoin/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@tachacoin.tech) and he will commit them.
